...
st.markdown("_Prototype v2.1 (nog bezig, laat bugs weten!)_")

# Session state key to store the uploaded file and processed portfolio data
...
uploaded_file = st.file_uploader("Upload je Degiro CSV bestand 'Transactions.csv'", type=['csv'])

# Use session state to persist the uploaded file and processed data
if uploaded_file:
    st.session_state['uploaded_file'] = uploaded_file

# If no file was uploaded, show a message and stop execution
if st.session_state['uploaded_file'] is None:
    st.info("Upload je Degiro CSV Transactions (!) bestand om te beginnen.")
    st.stop()

# Load the file data only if it's not already cached in session state
if st.session_state['portfolio_df'] is None:
    ...
    # Save portfolio data to session state to persist it
    st.session_state['portfolio_df'] = portfolio.portfolio_df

# Display the graph if portfolio data exists in session state
if st.session_state['portfolio_df'] is not None:
    st.line_chart(st.session_state['portfolio_df']['Portfolio Value'])